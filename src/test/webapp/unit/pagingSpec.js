define([
    'angular', 'stng/globalScope', 'stng/settings'
], function(angular, globalScope, settings) {
    describe("arrayPaging", function() {
        ...
        it('should use the default page size if no page size defined', function() {
            var l = [-1,-2];
            for (var i = 0; i < settings.listPageSize; i++) {
                l.push(i);
            }
            var pagedList = angular.Array.paged(l);
            expect(pagedList.length).toEqual(settings.listPageSize);
            expect(normalize(pagedList)).toEqual(l.slice(0, settings.listPageSize));
        });
